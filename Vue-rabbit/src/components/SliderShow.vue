    ...
    slides: {
        ...
        validator: (value) => {
            ...
            return Array.isArray(value) && (
                value.length === 0 ||
                value.every(item => item && (item.src || item.imgUrl))
            )
        }
    },
    width: {
...
// 统一清理与适配字段：支持 imgUrl/hrefUrl
const normalizeUrl = (u) => (u || '').replace(/`/g, '').trim()
const resolvedSlides = computed(() => (props.slides || []).map(item => ({
    src: normalizeUrl(item.src || item.imgUrl),
    path: normalizeUrl(item.path || item.hrefUrl),
    alt: item.alt ?? item.id
})))
const isInternalPath = (p) => !!p && p.startsWith('/')
...
<template>
    <div class="carousel-container" :style="{ width: containerWidth, height: containerHeight }">
        <div class="carousel-wrapper" :style="{
            ...
            <div class="carousel-slide" v-for="(slide, index) in resolvedSlides" :key="index">
                <!-- 内部路由：使用 RouterLink；外部链接：使用 <a>；无链接：仅图片 -->
                <template v-if="slide.path && isInternalPath(slide.path)">
                    <RouterLink :to="slide.path">
                        <img :src="slide.src" :alt="slide.alt || `${index + 1}`" class="slide-image">
                    </RouterLink>
                </template>
                <template v-else-if="slide.path">
